# bot_otc.py - MSL ACADEMY BOT - OTC VERSION
import os
import asyncio
import sqlite3
import uuid
...
async def execute_signal(data_source, sig):
    ...
    try:
        ...
        await asyncio.sleep(120 if not FAST_MODE else 10)
        ...
        # ===== MG1 ENTRY =====
        # ALWAYS execute MG1 - wait 3 minutes after first entry expiry
        ...
        # ===== MG2 ENTRY =====
        # ALWAYS execute MG2 - wait 3 minutes after MG1 entry expiry
...
async def run_trading_session():
    ...
    greeting = f"""📊 MSL ACADEMY BOT - OTC SESSION STARTED
...
🔥 Starting in 30 seconds..."""
    ...
    for sig_num in range(1, SIGNALS_PER_SESSION + 1):
        try:
            ...
            if result == "WIN":
                stats["wins"] += 1
                if win_type == "FIRST":
                    stats["first_entry_wins"] += 1
                    print(f"[STATS] WIN at FIRST ENTRY")
                elif win_type == "MG1":
                    stats["mg1_wins"] += 1
                    print(f"[STATS] WIN at MG1")
                elif win_type == "MG2":
                    stats["mg2_wins"] += 1
    ...
    summary = f"""📊 MSL ACADEMY BOT - OTC SESSION COMPLETED
...
if __name__ == "__main__":
    print("🔥 MSL ACADEMY BOT v2.0 - OTC REAL DATA VERSION")
    print("=" * 60)
